...
from schema.graph import run_migrations
...
async def test_run_migrations_executes_all_statements() -> None:
    ...
    assert any(
        "CREATE" in call.args[1] for call in mock_client.execute_command.call_args_list
    )
...
async def test_run_migrations_handles_empty_directory() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        with patch("schema.graph.MIGRATIONS_DIR", Path(tmpdir)):
            await run_migrations(mock_client, "testdb")
