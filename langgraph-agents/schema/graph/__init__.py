"""Graph schema migration runner.

Executes graph migrations idempotently against ArcadeDB.
...
async def run_migrations(client: ArcadeDBClientProtocol, database: str) -> None:
    """Execute all graph migrations in filename order.
    ...
    if not migration_files:
        logger.info("No graph migrations found")
        return
