...
resource "aws_bedrockagent_agent" "coding_agent" {
  ...
  instruction             = "Execute approved implementation plans."
}
...
resource "aws_ecr_repository" "exploratory_agent" {
  name                 = "agent-ops/exploratory-agent"
  image_tag_mutability = "MUTABLE"
  force_delete         = true
  lifecycle {
    prevent_destroy = false
...
resource "aws_bedrockagent_agent" "exploratory_agent" {
  ...
  agent_resource_role_arn = aws_iam_role.agentcore_role.arn
  foundation_model        = "anthropic.claude-v2"
  instruction             = "Execute exploratory mandates by investigating domains and emitting novel signals."
}
...
resource "aws_lambda_function" "exploratory_agent_dispatcher" {
  ...
  role          = aws_iam_role.agentcore_role.arn
  package_type  = "Image"
  image_uri     = "${aws_ecr_repository.exploratory_agent.repository_url}:latest"
  timeout       = 900
  memory_size   = 2048
  environment {
    variables = merge({
