"""Identity schema migration runner and version.

Executes identity migrations idempotently against ArcadeDB.
All migration SQL uses CREATE DOCUMENT TYPE IF NOT EXISTS, CREATE PROPERTY
IF NOT EXISTS, and CREATE INDEX IF NOT EXISTS, making the runner safe to
...
async def run_migrations(client: ArcadeDBClientProtocol, database: str) -> None:
    """Execute all identity migrations in filename order.
    ...
    if not migration_files:
        logger.info("No identity migrations found")
        return
